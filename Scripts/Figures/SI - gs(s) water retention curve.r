...
# Data
data1   <- read.csv("Results/SI-gs-water-retention-curve-1.csv")
data2   <- read.csv("Results/SI-gs-water-retention-curve-2.csv")
...
plot(0, 0, type="n", xaxt="n", yaxt="n", xlab=NA, ylab=NA,
     xlim=c(-10, 0), ylim=c(0, 0.3), cex.lab=1.3, main = expression(ESS~italic(g[s]*(s))))
points(data1, type="l")
points(data2, type="l", col=2)

axis(1, xlim=c(-10, 0), pos=0, lwd=2)
mtext(expression(italic(s)),side=1, line=2, cex=1.3)
axis(2, ylim=c(0, 0.3), pos=-10, lwd=2, at = c(0, 0.1, 0.2, 0.3))
